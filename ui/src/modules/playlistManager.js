...
export class PlaylistManager {
  ...
  async fetchPlaylists() {
    ...
    try {
      ...
      const response = await fetch(`${SERVER_URL}/me/playlists`, {
        headers: {
  ...
  renderPlaylists(playlists) {
    ...
    playlists.forEach(playlist => {
      ...
      li.dataset.uri = playlist.uri;

      const img = document.createElement('img');
      img.className = 'playlist-item-art';
      img.alt = playlist.name;
      const artUrl = playlist.images?.[0]?.url || URLS.PLACEHOLDER_ALBUM_ART;
      img.src = artUrl;
      li.appendChild(img);
  ...
  async fetchPlaylistTracks(playlistId) {
    ...
    try {
      const response = await fetch(`${SERVER_URL}/playlists/${encodeURIComponent(playlistId)}/tracks`, {
        headers: {
